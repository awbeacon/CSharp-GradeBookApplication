...
namespace GradeBook.GradeBooks
{
    public abstract class BaseGradeBook
    {
        ...
        public void RemoveStudent(string name)
        {
            ...
            Student student = Students.FirstOrDefault(e => e.Name == name);
            if (student == null)
            {
        ...
        public static BaseGradeBook Load(string name)
        {
            ...
            using (FileStream file = new FileStream(name + ".gdbk", FileMode.Open, FileAccess.Read))
            {
        ...
        public void Save()
        {
            using (FileStream file = new FileStream(Name + ".gdbk", FileMode.Create, FileAccess.Write))
            {
        ...
        {
            Double GPA = (!(studentType==StudentType.Standard) && IsWeighted) ? 1:0;
        ...
        public static dynamic ConvertToGradeBook(string json)
        {
            ...
            Type gradebookEnum = (from assembly in AppDomain.CurrentDomain.GetAssemblies()
                                  from type in assembly.GetTypes()
                                  where type.FullName == "GradeBook.Enums.GradeBookType"
                                  select type).FirstOrDefault();

            JObject jobject = JsonConvert.DeserializeObject<JObject>(json);
            string gradeBookType = jobject.Property("Type")?.Value?.ToString();
            ...
            Type gradebook = (from assembly in AppDomain.CurrentDomain.GetAssemblies()
                              from type in assembly.GetTypes()
